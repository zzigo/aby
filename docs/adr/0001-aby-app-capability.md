...
4. Wasabi stays private, with canonical keys under `aby/aud/` and `aby/mov/` and playback through short-lived presigned URLs. Legacy `ref/` and `mov/` objects are adopted only through preview and verified promotion as refined by ADR 0002.
