<script lang="ts">
  ...
  let languagesText = $state(''); let summary = $state(''); let posterUrl = $state(''); let creditsText = $state('');
  ...
  onMount(() => {
    if (canInspect) void loadOperations();
  ...
  function applyInspection(next: Inspection) {
    ...
    director = next.embeddedMetadata.director ?? ''; country = next.embeddedMetadata.country ?? '';
    languagesText = next.embeddedMetadata.languages.join(', '); summary = next.embeddedMetadata.summary ?? '';
    if (!treeValue) treeValue = director || 'unknown';
  ...
  async function inspectSource(source: Source) {
    ...
    try { const body = await jsonRequest('/api/av/inspect', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({sourceObjectKey:source.objectKey}) }); applyInspection(body.inspection); }
  ...
  async function reloadService(service: Service) {
    if (!title.trim()) { status = 'Enter a title before querying metadata.'; return; }
    serviceState[service] = 'LOADING'; status = `Rechecking ${service}…`;
    try {
      const params = new SvelteURLSearchParams({ q:title.trim(), service }); if (year) params.set('year',String(year));
      ...
      status = candidates[service].length ? `${candidates[service].length} ${service} candidates. Canonical fields are unchanged until USE.` : `${service} returned no candidates.`;
    } catch (error) { serviceState[service] = 'ERROR'; status = error instanceof Error ? error.message : `${service} failed`; }
  }

  function updateCreditsText(next: AvCredit[]) { creditsText = next.map((credit) => [credit.name, credit.role, credit.character].filter(Boolean).join(' | ')).join('\n'); }
  function parseCredits() { return creditsText.split('\n').map((line) => line.split('|').map((part) => part.trim())).filter(([name,role]) => name && role).map(([name,role,character]) => ({ name:name!, role:role!, ...(character?{character}:{}), externalIds:{} })); }
  ...
  async function useCandidate(candidate: AvMetadataCandidate) {
    ...
    if (candidate.authority === 'tmdb') {
      ...
      if (body.details) { director = body.details.director || director; country = body.details.country || country; languagesText = (body.details.languages ?? []).join(', ') || languagesText; imdbId = body.details.externalIds?.imdb ?? imdbId; updateCreditsText(body.details.credits ?? []); }
    }
  ...
  async function commitToCatalog() {
    ...
    try {
      ...
      const body = await jsonRequest('/api/av/items', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({
        ...
        ...(year?{year}:{}), ...(director.trim()?{director:director.trim()}:{}), ...(entity.trim()?{entity:entity.trim()}:{}), ...(saga.trim()?{saga:saga.trim()}:{}),
        ...(country.trim()?{country:country.trim()}:{}), languages:languagesText.split(',').map((value)=>value.trim()).filter(Boolean), ...(summary.trim()?{summary:summary.trim()}:{}),
        ...(posterUrl.trim()?{posterUrl:posterUrl.trim()}:{}), credits:parseCredits(), externalIds, metadataSources, treeStrategy, treeValue:strategyValue().trim(), destinationObjectKey:'server-proposed'
      }) });
    ...
    <section class="canonical-editor">
      ...
      {#if inspection}<div class="editor-body">
        ...
        <div class="fields">
          ...
          <div class="quad"><label>KIND<select bind:value={kind}><option value="film">FILM</option><option value="episode">EPISODE</option><option value="video">VIDEO</option><option value="archive">ARCHIVE</option><option value="personal">PERSONAL</option></select></label><label>YEAR<input bind:value={year} type="number" min="1800" max="2200" /></label><label>DIRECTOR<input bind:value={director} /></label><label>COUNTRY<input bind:value={country} /></label></div>
          <div class="wide"><label>ENTITY<input bind:value={entity} /></label><label>SAGA<input bind:value={saga} /></label></div>
          <label>LANGUAGES · COMMA SEPARATED<input bind:value={languagesText} /></label><label>SUMMARY<textarea bind:value={summary} rows="6"></textarea></label>
          <div class="ids"><label>IMDb ID<input bind:value={imdbId} /></label><label>TMDB ID<input bind:value={tmdbId} /></label><label>WIKIDATA ID<input bind:value={wikidataId} /></label><label>INTERNET ARCHIVE ID<input bind:value={internetArchiveId} /></label></div>
          <label>CREDITS · ONE PER LINE · NAME | ROLE | CHARACTER<textarea bind:value={creditsText} rows="7"></textarea></label>
          <div class="tree"><label>TREE<select bind:value={treeStrategy}><option value="author">AUTHOR</option><option value="decade">DECADE</option><option value="entity">ENTITY</option><option value="saga">SAGA</option><option value="custom">CUSTOM</option></select></label><label>TREE VALUE<input bind:value={treeValue} /></label></div>
...
<style>
  .av-inspect{max-width:1500px;margin:0 auto;padding:64px 28px 140px}.av-intro{display:grid;grid-template-columns:1fr minmax(280px,500px);gap:40px;align-items:end;margin-bottom:54px}.av-intro span,.source-step header span,.inspect-grid>article>header span,.canonical-editor>header span,.operation-thread>header span{color:var(--signal);font:9px ui-monospace,monospace;letter-spacing:.12em}.av-intro h1{margin:8px 0 0;font:400 clamp(38px,6vw,82px)/.95 Georgia,serif}.av-intro p{color:var(--muted);line-height:1.6}.source-step,.source-preview,.authority-panel,.canonical-editor,.operation-thread,.signin{border:1px solid var(--line);background:var(--surface)}.source-step>header,.inspect-grid>article>header{padding:15px 18px;display:flex;align-items:baseline;gap:14px;border-bottom:1px solid var(--line)}h2{margin:0;font-size:17px;font-weight:500}.source-step header small,.inspect-grid header small{margin-left:auto;color:var(--muted);font:9px ui-monospace,monospace}.source-actions{padding:16px;display:flex;gap:8px}.source-actions button,.signin button{min-height:40px;padding:0 18px;border:1px solid var(--line);background:transparent;color:#fff;font:10px ui-monospace,monospace}.source-actions .primary,.signin button{border-color:var(--signal);color:var(--signal)}.source-browser{max-height:360px;margin:0 16px 16px;border:1px solid var(--line);overflow:auto}.source-browser>div{position:sticky;top:0;display:grid;grid-template-columns:1fr auto;background:#10120f}.source-browser input{padding:10px;border:0;background:#090a09;color:#fff}.source-browser>div button{border:0;background:#171916;color:var(--signal)}.source-browser>small{display:block;padding:7px 10px;color:var(--muted);font:8px ui-monospace,monospace}.source-browser>button{width:100%;padding:9px 10px;display:grid;gap:3px;border:0;border-top:1px solid var(--line);background:transparent;color:#fff;text-align:left}.source-browser>button strong{font-size:10px}.source-browser>button span{color:var(--muted);font:8px ui-monospace,monospace;overflow-wrap:anywhere}.inspect-grid{margin-top:18px;display:grid;grid-template-columns:minmax(0,1fr) minmax(390px,.8fr);gap:18px}.source-preview video{width:100%;max-height:440px;background:#000}.source-preview dl{margin:0;display:grid;grid-template-columns:repeat(2,1fr)}.source-preview dl div{padding:10px 13px;border-top:1px solid var(--line)}dt{color:var(--muted);font:8px ui-monospace,monospace}dd{margin:4px 0 0;overflow-wrap:anywhere;font:10px ui-monospace,monospace}.source-preview details{border-top:1px solid var(--line)}.source-preview summary{padding:12px;color:var(--signal);font:9px ui-monospace,monospace}.source-preview pre{max-height:240px;margin:0;padding:12px;overflow:auto;color:var(--muted);font:9px ui-monospace,monospace}.authority{border-top:1px solid var(--line)}.authority>header{padding:10px 12px;display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center}.authority>header strong{font:9px ui-monospace,monospace}.authority>header span{color:var(--muted);font:8px ui-monospace,monospace}.authority button{border:1px solid var(--line);background:transparent;color:var(--signal);padding:7px;font:8px ui-monospace,monospace}.authority>p{padding:0 12px 10px;color:var(--muted);font:9px ui-monospace,monospace}.authority>div{max-height:220px;overflow:auto}.authority article{padding:10px 12px;border-top:1px solid #282b26}.authority article>span{float:right;color:var(--signal);font:8px ui-monospace,monospace}.authority article strong{font-size:11px}.authority article p{max-height:34px;margin:4px 0;color:var(--muted);overflow:hidden;font-size:9px;line-height:1.4}.authority article footer{display:flex;justify-content:space-between;align-items:center}.authority article a{color:var(--muted);font:8px ui-monospace,monospace}.canonical-editor{margin-top:18px}.canonical-editor>header,.operation-thread>header{padding:18px;display:flex;justify-content:space-between;align-items:baseline;border-bottom:1px solid var(--line)}.canonical-editor>header>div,.operation-thread>header>div{display:flex;gap:14px;align-items:baseline}.canonical-editor>header p{max-width:560px;margin:0;color:var(--muted);font-size:11px}.editor-body{display:grid;grid-template-columns:240px 1fr}.poster-column{padding:18px;border-right:1px solid var(--line)}.poster{aspect-ratio:2/3;margin-top:12px;display:grid;place-items:center;overflow:hidden;background:#20231d;color:var(--signal);font:60px Georgia,serif}.poster img{width:100%;height:100%;object-fit:cover}.fields{padding:18px;display:grid;gap:12px}.fields label,.poster-column label{display:grid;gap:5px;color:var(--muted);font:8px ui-monospace,monospace}.fields input,.fields textarea,.fields select,.poster-column input{width:100%;box-sizing:border-box;padding:9px;border:1px solid var(--line);background:#090a09;color:#fff;font:11px ui-monospace,monospace}.wide,.quad,.ids,.tree{display:grid;gap:10px}.wide{grid-template-columns:1fr 1fr}.quad,.ids{grid-template-columns:repeat(4,1fr)}.tree{grid-template-columns:180px 1fr}.provenance{display:flex;flex-wrap:wrap;gap:5px}.provenance span{padding:5px 7px;background:#1b1d19;color:var(--muted);font:8px ui-monospace,monospace}.commit{min-height:44px;border:1px solid var(--signal);background:transparent;color:var(--signal);font:10px ui-monospace,monospace}.commit:disabled{opacity:.35}.view-link{justify-self:start;color:var(--signal);font:10px ui-monospace,monospace}.operation-thread{margin-top:18px}.operation-thread>header small{color:var(--muted);font:8px ui-monospace,monospace}.operation-thread>article{padding:13px 18px;border-bottom:1px solid var(--line)}.operation-thread>article>div:first-child{display:grid;gap:3px}.operation-thread article strong,.operation-thread article span,.operation-thread article small{overflow-wrap:anywhere;font:9px ui-monospace,monospace}.operation-thread article span,.operation-thread article small{color:var(--muted)}.metrics{margin:9px 0;display:grid;grid-template-columns:repeat(5,1fr)}.operation-thread article>i{display:block;height:2px;background:#2c2f29}.operation-thread article>i b{display:block;height:100%;background:var(--signal)}.operation-thread article footer{margin-top:8px;display:flex;justify-content:space-between;align-items:center}.operation-thread article footer button{padding:7px 11px;border:1px solid var(--signal);background:transparent;color:var(--signal);font:9px ui-monospace,monospace}.empty{padding:24px;color:var(--muted);line-height:1.5}.status{position:fixed;z-index:30;left:18px;bottom:76px;max-width:min(760px,calc(100vw - 36px));padding:10px 13px;display:flex;gap:9px;align-items:center;background:#111310f2;border:1px solid var(--line);font:10px ui-monospace,monospace}.status i{width:7px;height:7px;border-radius:50%;background:var(--signal)}.status i.working{animation:pulse .8s infinite alternate}.signin{padding:28px}.signin button{margin-top:10px}@keyframes pulse{to{opacity:.2}}
