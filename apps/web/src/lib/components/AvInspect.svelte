<script lang="ts">
  ...
  async function reloadService(service: Service) {
    ...
    try {
      ...
      if (id) params.set('id', id); else params.set('q', title.trim());
      ...
      status = candidates[service].length ? `${candidates[service].length} ${service} candidates. Canonical fields are unchanged until USE.` : `${service} returned no candidates for ${id || `${title}${year ? ` (${year})` : ''}`}.`;
        ...
        {#each services as service (service.id)}
          <section class="authority"><header><strong>{service.label}</strong><span>{serviceState[service.id]}</span><button onclick={() => reloadService(service.id)} disabled={!inspection || busy}>↻ RELOAD</button></header>
            <div class="authority-id">
              {#if service.id === 'tmdb'}<label>TMDB ID<input bind:value={tmdbId} placeholder="Blank searches canonical title + year" /></label>
              {:else if service.id === 'wikidata'}<label>WIKIDATA ID<input bind:value={wikidataId} placeholder="Q… or blank for title search" /></label>
              {:else}<label>INTERNET ARCHIVE ID<input bind:value={internetArchiveId} placeholder="Identifier or blank for title search" /></label>{/if}
              <small>{service.id === 'tmdb' ? 'Numeric movie ID' : service.id === 'wikidata' ? 'Q identifier' : 'Archive item identifier'} · automatically filled after USE</small>
            </div>
